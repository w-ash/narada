...
from narada.core.protocols import connector_metrics, metric_resolvers
...
async def process_metrics_for_track(
    session: AsyncSession, track_id: int, connector: str, metadata: dict[int, dict[str, Any]] | None = None
) -> dict[str, Any]:
...
async def resolve_connector_metrics(track_id: int, connector: str) -> dict[str, Any]:
    ...
    try:
        ...
        async with get_session() as session:
            ...
            results = await process_metrics_for_track(session, track_id, connector, metadata)
        ...
        logger.error(
            f"Error resolving metrics for track {track_id} from {connector}: {e}",
            exc_info=True,
        )
