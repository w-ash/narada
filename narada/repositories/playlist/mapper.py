    ...
    async def to_domain(db_model: DBPlaylist) -> Playlist:
        ...
        for pt in active_tracks:
            ...
            for m in track_mappings:
                ...
                # Skip if no connector track found
                ...
                if not connector_track or (
                    hasattr(connector_track, "is_deleted")
                    and connector_track.is_deleted
                ):
                    continue

                # Skip if it doesn't have needed attributes
                if not hasattr(connector_track, "connector_name") or not hasattr(
            ...
            if not all(hasattr(track, attr) for attr in required_attributes):
                continue
        ...
        for m in playlist_mappings:
            # Skip deleted mappings or those missing essential attributes
            if (
                hasattr(m, "is_deleted")
