...
class LikeService:
    ...
    async def _import_spotify_likes_internal(
        ...
        while True:
            ...
            (
                connector_tracks,
                next_cursor,
            ) = await self.spotify_connector.get_liked_tracks(
            ...
            for connector_track in connector_tracks:
                try:
                    ...
                    existing_track = (
                        await self.repositories.connector.find_track_by_connector(
                            connector="spotify",
                    ...
                    if existing_track and existing_track.id is not None:
                        ...
                        if await self._is_track_already_liked(
                            existing_track.id, ["spotify", "narada"]
                        ):
    ...
    async def _export_likes_to_lastfm_internal(
        ...
        total_liked_in_narada = len(
            await self.repositories.likes.get_all_liked_tracks(
    ...
    def _create_error_result(
        self, error_message: str, batch_id: str = ""
    ) -> OperationResult:
