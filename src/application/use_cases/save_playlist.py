...
class SavePlaylistCommand:
    ...
    def validate(self) -> bool:
        ...
        if (self.persistence_options.operation_type == "update_spotify" 
...
class SavePlaylistUseCase:
    ...
    enrichment_strategy: TrackEnrichmentStrategy = field(
        factory=BasicEnrichmentStrategy
    )
    ...
    async def _create_internal_playlist(
        ...
        playlist = create_playlist_operation(
            ...
            {"name": options.playlist_name, "description": options.playlist_description},
            tracks
        )
    ...
    async def _create_spotify_playlist(
        ...
        from src.domain.workflows.playlist_operations import create_spotify_playlist_operation
        
        if not options.spotify_playlist_id:
            raise ValueError("Spotify playlist ID required for create_spotify operation")
    ...
    async def _update_spotify_playlist(
        ...
        from src.domain.workflows.playlist_operations import update_playlist_tracks_operation
        
        if not options.spotify_playlist_id:
            raise ValueError("Spotify playlist ID required for update_spotify operation")
        ...
        updated = update_playlist_tracks_operation(existing, tracks, options.append_mode)
