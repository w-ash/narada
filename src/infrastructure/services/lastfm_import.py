...
class LastfmImportService(BaseImportService):
            ...
            return OperationResult(
                ...
                play_metrics={
                    **result.play_metrics,
                    ...
                    "from_timestamp": self._incremental_from_timestamp.isoformat() if self._incremental_from_timestamp else None,
                    "to_timestamp": self._incremental_to_timestamp.isoformat() if self._incremental_to_timestamp else None,
                },
        ...
        **kwargs
        ...
        while records_fetched < limit:
            ...
            if progress_callback:
                progress = 20 + int((records_fetched / limit) * 40)  # 20-60% for API fetching
                progress_callback(
                    progress, 
                    100, 
                    f"Fetching page {current_page} from Last.fm API..."
                )
            ...
            if len(page_records) < 200:  # Last.fm API page size
                logger.info(f"Reached end of data with {len(page_records)} records on page {current_page - 1}")
                break
    ...
    async def _fetch_incremental_strategy(
        ...
        **kwargs
        ...
        if not username:
            raise ValueError("No Last.fm username provided or configured (set LASTFM_USERNAME environment variable)")
        ...
        while True:
            if progress_callback:
                ...
                progress_callback(
                    progress, 
                    100, 
                    f"Fetching page {page} from Last.fm API..."
                )
    ...
    async def _process_with_resolution(
        ...
        **kwargs
        ...
        logger.info(f"Track resolution completed: {resolved_count} resolved, {unresolved_count} unresolved")
        return track_plays
        ...
        if strategy == "incremental":
            ...
            if username:
                ...
                if raw_data:
                    ...
                    logger.info(f"Updated checkpoint for {username} to {most_recent_timestamp}")
                    ...
                    await self.repositories.checkpoints.save_sync_checkpoint(checkpoint)
                    logger.info(f"Updated checkpoint for {username} to current time (no new data)")
    ...
    async def _resolve_tracks_from_play_records(
        ...
        logger.info(f"Resolved {len(resolution_map)} out of {len(play_records)} play records")
