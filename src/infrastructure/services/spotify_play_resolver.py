...
class PlayResolution:
    ...
    spotify_uri: str
    track_id: int | None
    resolution_method: str  # "direct_id", "relinked_id", "search_match", "preserved_metadata"
    confidence: int | None
...
class SpotifyPlayResolver:
    ...
    async def resolve_play_records(
        self, 
        play_records: list[SpotifyPlayRecord]
    ) -> dict[str, int]:
        """
        ...
        spotify_tracks = await self.spotify_connector.get_tracks_by_ids(unique_track_ids)
        ...
        for record in play_records:
            ...
            artists = [Artist(name=artist["name"]) for artist in spotify_data.get("artists", [])]
        ...
        track_ids = [extract_spotify_track_id(record.track_uri) for record in play_records]
        ...
        existing_tracks = await self.track_repos.connector.find_tracks_by_connectors(connections)
        ...
        logger.info(f"Resolved {len(uri_to_id_map)} out of {len(play_records)} tracks from existing connector mappings")
    ...
    async def resolve_play_records_with_creation(
        ...
        unresolved_records = [
            record for record in play_records 
            if record.track_uri not in uri_to_id_map
        ]
        ...
        logger.info(f"Creating {len(unresolved_records)} missing tracks from Spotify API")
        
        # Step 3: Get track data from Spotify API (already fetched in resolve_play_records)
        unique_track_ids = [extract_spotify_track_id(record.track_uri) for record in unresolved_records]
        spotify_tracks = await self.spotify_connector.get_tracks_by_ids(unique_track_ids)
        ...
        logger.info(f"Creating {len(connector_tracks)} tracks using existing infrastructure")
        ...
        for record, created_track in zip(unresolved_records, created_tracks, strict=True):
            if created_track and created_track.id is not None:
                uri_to_id_map[record.track_uri] = created_track.id
                logger.debug(f"Created track {created_track.id} for URI {record.track_uri}")
        
        logger.info(f"Enhanced resolution: {len(uri_to_id_map)} total tracks resolved ({len(created_tracks)} newly created)")
        return uri_to_id_map
    ...
    async def resolve_with_fallback(
        ...
        logger.info(f"Starting comprehensive resolution for {len(play_records)} play records")
        ...
        if unresolved_uris:
            logger.info(f"Stage 3: Preserving metadata for {len(unresolved_uris)} unresolved URIs")
            
            unresolved_records = [r for r in play_records if r.track_uri in unresolved_uris]
    ...
    async def _resolve_direct_with_relinking(
        ...
        for uri in unique_uris:
            try:
                ...
                if len(track_id) == 22 and track_id.isalnum():
                    ...
                    logger.warning(f"Invalid track ID format: {track_id} from URI: {uri}")
        ...
        logger.info(f"Direct lookup resolved {len(resolution_results)}/{len(unique_uris)} URIs")
        return resolution_results
    
    async def _resolve_via_search(
        self, 
        failed_records: list[SpotifyPlayRecord]
    ) -> dict[str, PlayResolution]:
            ...
            try:
                ...
                if search_result and search_result.get("id"):
                    ...
                    confidence, evidence = calculate_confidence(
                        original_track, search_track_data, "artist_title"
                    )
                    ...
                    if confidence >= 70:
                        ...
                        connection_key = ("spotify", spotify_id)
                        existing_tracks = await self.track_repos.connector.find_tracks_by_connectors([connection_key])
                        ...
                        logger.debug(f"Search match found for {record.track_uri}: confidence {confidence}%")
                    else:
                        logger.debug(f"Low confidence search result for {record.track_uri}: {confidence}%")
                
            except Exception as e:
                logger.warning(f"Search failed for {record.track_uri}: {e}")
        
        logger.info(f"Search fallback resolved {len(resolution_results)}/{len(failed_records)} records")
        return resolution_results
    
    def _preserve_metadata(
        self, 
        unresolved_records: list[SpotifyPlayRecord]
    ) -> dict[str, PlayResolution]:
        ...
        logger.info(f"Preserved metadata for {len(resolution_results)} unresolved records")
        return resolution_results
    
    def _calculate_resolution_stats(
        self, 
        resolution_results: dict[str, PlayResolution]
    ) -> dict[str, int]:
    ...
    async def _create_track_from_spotify_data(
        ...
        try:
            ...
            if created_tracks and len(created_tracks) > 0 and created_tracks[0].id is not None:
                logger.debug(f"Created internal track {created_tracks[0].id} for Spotify ID {spotify_id}")
                return created_tracks[0].id
            else:
                logger.warning(f"Failed to create internal track for Spotify ID {spotify_id}")
                return None
