...
class RichProgressProvider:
    ...
    def set_description(self, operation_id: str, description: str) -> None:
        ...
        with self._lock:
            ...
            self._progress.update(
                task_id, description=f"[{style}]{description}[/{style}]"
            )
    ...
    def complete_operation(self, operation_id: str) -> None:
        ...
        with self._lock:
            ...
            if "[cyan]" in base_description:
                base_description = base_description.replace("[cyan]", "").replace(
                    "[/cyan]", ""
                )
            elif "[dim cyan]" in base_description:
                base_description = base_description.replace("[dim cyan]", "").replace(
                    "[/dim cyan]", ""
                )
