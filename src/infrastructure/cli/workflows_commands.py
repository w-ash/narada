...
def run(
    workflow_id: Annotated[
        str | None,
        typer.Argument(help="Workflow ID to execute", autocompletion=complete_workflow_names),
    ] = None,
    ...
    if not workflow_id:
        ...
        try:
            ...
            matching_workflow = next(
                (wf for wf in workflows if wf["id"].lower() == choice_input.lower()), None
            )
...
def _simple_workflow_feedback(event_type: str, event_data: dict) -> None:
    ...
    match event_type:
        case "workflow_started":
            ...
            console.print(f"[bold blue]▶ Starting workflow:[/bold blue] [bold]{workflow_name}[/bold]")
        
        case "workflow_completed":
            workflow_name = event_data.get("workflow_name", "Unknown workflow")
            console.print(f"[bold green]✓ Workflow completed:[/bold green] [bold]{workflow_name}[/bold]")
...
def _get_completion_summary(result: dict) -> str:
    ...
    if "tracklist" in result:
        track_count = len(result["tracklist"].tracks) if hasattr(result["tracklist"], "tracks") else 0
...
def list_workflows():
    ...
    definitions_path = current_file.parent.parent.parent / "application" / "workflows" / "definitions"
    workflows = []
    ...
    for json_file in definitions_path.glob("*.json"):
        try:
            ...
            console.print(f"[yellow]Warning: Could not parse {json_file.name}: {e}[/yellow]")
            continue
